...
HRESULT CSound_Manager::NativeConstruct(const _tchar* pSoundFilePath)
{
	FMOD::System* pSystem = nullptr;
	FMOD::System_Create(&pSystem);

	m_pSystem = (FMOD_SYSTEM*)pSystem;
	if (nullptr == m_pSystem) 
	{
		MSG_BOX(L"Failed To CSound_Manager : NativeConsruct : nullptr == m_pSystem");
		return E_FAIL;
	}
...
void CSound_Manager::LoadSoundFile(const _tchar* pSoundFilePath)
{
	_tfinddata64_t fd;
	_tchar szBuff[MAX_PATH] = L"";
	lstrcpy(szBuff, pSoundFilePath);
	lstrcat(szBuff, L"*.wav");
	__int64 handle = _tfindfirst64(szBuff, &fd);
	...
	char szCurPath[128] = "";
	WideCharToMultiByte(CP_ACP, 0, pSoundFilePath, -1, szCurPath, sizeof(szCurPath), NULL, NULL);
	char szFullPath[128] = "";
	...
	while (iResult != -1) 
	{
		...
		if (eRes == FMOD_OK)
		{
			int iLength = _uint(strlen(szFilename) + 1);
